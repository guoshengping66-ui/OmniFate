...
return(<div className="w-full text-white" style={{background:"#000"}}>
  ...
  {/* ═══ L4: Vignette ═══ */}
  <div className="fixed inset-0 pointer-events-none" aria-hidden="true" style={{background:"radial-gradient(circle at 50% 45%, rgba(0,0,0,0.10), transparent 50%), linear-gradient(180deg, rgba(0,0,0,0.06) 0%, transparent 18%, transparent 52%, rgba(0,0,0,0.50) 100%)",zIndex:8}}/>
  <style>{kf}</style>

  {/* ═══ HERO ═══ */}
  <section className="relative flex min-h-[88vh] w-full flex-col items-center justify-center px-6 text-center" style={{zIndex:10}}>
    <div><h1 className="font-serif text-8xl md:text-9xl font-bold" style={{background:"linear-gradient(180deg, #f1d36f 0%, #c49a35 58%, #8d6a1f 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",textShadow:"0 0 32px rgba(218,180,74,0.18)"}}>{isZh?"观我":"Guanwo"}</h1><p className="mt-4 text-[11px] tracking-[0.15em] text-white/30">{isZh?"AI 命运行动系统":"AI Destiny Action System"}</p></div>
    <div className="mt-10"><p className="font-serif text-xl" style={{color:"rgba(238,241,232,0.82)"}}>{isZh?"看见内在结构 · 找到下一步":"See your structure · Find the next step"}</p><div className="mx-auto mt-4 h-px w-48" style={{background:"linear-gradient(90deg, transparent, rgba(255,255,255,0.12), transparent)"}}/><p className="mx-auto mt-4 max-w-md text-[14px] leading-relaxed" style={{color:"rgba(220,224,215,0.58)"}}>{isZh?"融合八字、紫微、星盘、塔罗、面相手相":"Integrating Bazi, Ziwei, Astrology, Tarot, Face & Palm"}<br/>{isZh?"AI 五维交叉验证，生成你的完整命运画像":"AI five-source cross-validation — your complete destiny profile"}</p></div>
    <div className="mt-12 flex flex-col items-center gap-4 sm:flex-row"><Link href={localeHref("/reading/new")} className="rounded-xl px-10 py-4 font-medium transition-all hover:scale-[1.03]" style={{background:"#C9A84C",color:"#020617"}}><span className="flex items-center gap-2">{isZh?"建立我的画像":"Build My Profile"} <ArrowRight size={16}/></span></Link><Link href={localeHref("/almanac")} className="rounded-xl border px-10 py-4 text-white/75 transition-all hover:border-white/35" style={{borderColor:"rgba(255,255,255,0.18)",background:"rgba(5,10,18,0.32)",backdropFilter:"blur(12px)"}}><span className="flex items-center gap-2">{isZh?"查看今日趋势":"View Today's Trend"} <ArrowRight size={15}/></span></Link></div>
