...
export const CATEGORIES = [
  ...
  { key: "four-pillar", label_zh: "八字分析", label_en: "Four-Pillar Analysis" },
  { key: "chart analysis", label_zh: "西方星盘", label_en: "Western Chart" },
  { key: "symbol", label_zh: "塔罗指南", label_en: "Symbol Guide" },
  { key: "face", label_zh: "面部手相", label_en: "Face & Hand" },
...
export const ARTICLES: Article[] = [
  ...
  {
    id: "wuxing-basics",
    title_zh: "五素入门：金木水火土如何影响你的命运",
    title_en: "Five Elements Basics: How Metal, Wood, Water, Fire & Earth Shape Your Profile",
    summary_zh: "五素是中国行为分析学的基础框架。了解五素的相生相克关系，是理解八字档案的第一步。本文用最通俗的语言解释五素的本质。",
    summary_en: "The Five Elements form the foundation of Chinese behavioral analysis. Understanding their generating and overcoming cycles is the first step to reading a Four-Pillar chart. This article explains the essence of the Five Elements in plain language.",
    category: "wuxing",
    tags_zh: ["五素", "入门", "基础"],
    ...
    content_zh: `五素——金、木、水、火、土——是中国古代哲学的核心概念，也是行为分析学的基础框架。它描述了宇宙万物相生相克的动态平衡关系。
...
在八字档案中，每个人的出生时间对应着天干地支，每个天干地支都带有五素属性。通过计算八字中五素的分布，分析师可以判断你五素的强弱——这就是「五素得分」。

比如，如果你八字中木特别旺而金很弱，就叫「木旺缺金」。但这并不意味着你一定要补金——还需要看你档案的「喜用神」。

常见误区

误区一：缺什么就补什么。这是最常见的误解。行为分析学讲究的是「喜用神」，即你档案最需要的元素，而非简单地补充缺失的元素。有时缺的元素恰恰是你不需要的。

误区二：五素越均衡越好。均衡是理想状态，但大多数人的档案都有偏旺或偏弱的元素。关键是找到对你最有利的平衡点。

小测试

你知道自己的日主天干是什么吗？日主天干是八字中代表你自己的那个字。如果你知道自己的出生年月日时，可以试试我们的 AI 分析工具，一键生成你的五素分析。点击「开始分析」获取你的五素分析报告。`,
...
In a Four-Pillar chart, each person's birth time corresponds to a set of Stems and Earthly Branches, each carrying a Five Element attribute. By calculating the distribution of elements in the chart, a practitioner can determine your elemental strengths — this is called the 「Five Element Score.」
...
Do you know your Day Master Stem? The Day Master is the character in your Four-Pillar chart that represents you. If you know your birth date and time, try our AI analysis tool to instantly generate your Five Element analysis. Click 「Start Reading」 to get your Five Element analysis report.`,
  },
...
在八字八字中，日柱的天干被称为"日主"（也叫"日元"或"命主"），它代表你自己。日主是整个八字档案的核心，所有其他元素都围绕日主展开分析。
...
通过 AI 分析工具，输入你的出生年月日时，系统会自动计算你的八字八字，并标注你的日主天干。你还可以看到五素得分、十维关系等详细分析。
    `,
    content_en: `
## What Is the Day Master?

In the Four Pillars of Four-Pillar, the Stem of the Day Pillar is called the "Day Master" (also known as "Ri Yuan" or "Ming Zhu"). It represents you. The Day Master is the core of the entire Four-Pillar chart — all other elements are analyzed in relation to it.
...
Use our AI analysis tool — enter your birth date and time, and the system will automatically calculate your Four Pillars and identify your Day Master. You'll also see your Five Element scores, Ten Gods relationships, and more detailed analysis.
  ...
  // ────────────────────────────────────────────────────
  {
    ...
    summary_zh: "很多人一听「八字缺金」就急着戴金饰。但行为分析学的逻辑远比「缺啥补啥」复杂。本文解析五素喜忌的正确理解方式。",
...
这是行为分析学中最广泛流传的误解。很多人在行为分析后听到「八字缺金」，就立刻去戴金项链、金戒指，以为这样就能弥补档案的不足。但事实远没有这么简单。
...
1. **不要自行补五素**：盲目补充可能适得其反
2. **了解你的喜用神**：这是最关键的一步
3. **通过多种途径调和**：颜色、方位、职业选择等都可以调和五素
4. **动态看待**：周期状态会改变五素的需求
    `,
    content_en: `
## Why "Supplement What You Lack" Is Wrong

This is the most widespread misconception in behavioral analysis. After a reading, many people hear "your Four-Pillar lacks Metal" and immediately rush to wear gold necklaces or rings, thinking this will compensate for their chart's deficiency. But reality is far more complex.
...
4. **Think dynamically** — annual cycle cycles change your elemental needs
  ...
  // ────────────────────────────────────────────────────
  {
    ...
    summary_zh: "十维是八字中描述人际关系和命运走向的核心系统。正官代表约束与责任，偏财代表意外之财。搞懂十维，就读懂了档案的语言。",
...
十维是八字分析中用来描述日主与其他天干之间关系的十个名称。它们基于五素生克关系，进一步细分为：生我者、我生者、克我者、我克者、同我者，每类又分阴阳，共十个。
...
**看组合**：十维之间的组合（如食神生财、官印相生）会形成特定的格局格局。
...
在西方星盘学中，星盘被分为十二个宫位，每个宫位代表人生的不同领域。宫位就像十二个舞台，行星在其中上演你的人生剧本。
...
→ Enter your birth information and AI will generate your complete house analysis
  ...
  // ────────────────────────────────────────────────────
  {
    ...
    summary_zh: "相位是行星之间的角度关系，决定了行星互动模式。合相是融合，对冲是张力，三分相是和谐。理解相位，就读懂了星盘的动力系统。",
...
相位是星盘中行星之间形成的角度。不同的角度代表不同的互动模式。主要相位有五种：
...
面相学（观相术）通过观察人的面相来推断性格和状态。中国传统面相学将面部分为多个宫位，每个宫位对应不同的人生领域。
...
传统面相需要多年经验积累，而 AI 面相分析通过 468 个面相点的精确提取，可以快速、客观地给出面相解读。
...
西方星盘学将十二星座分为四组，每组对应一个元素。元素决定了星座的基本性质和行为模式。
...
→ Our AI 5-Dimension Analysis combines star sign elements with Five Elements for more precise readings
  ...
  {
    id: "bazi-marriage",
    title_zh: "八字合婚：五素配对与婚姻缘分",
    title_en: "Four-Pillar Marriage Compatibility: Five Element Pairing & Relationship Profile",
    summary_zh: "你和另一半的五素是相生还是相克？八字合婚不只是看属相，更要看日主配合、十维互补。本文教你用行为分析的视角理解婚恋关系。",
...
日主代表你自己，配偶日柱的地支代表你的配偶宫。看两人的日主五素关系是相生还是相克：
...
| 同类五素 | 比肩/劫财 | 朋友式婚姻，平等互助 |
| 相克五素 | 官杀/财星 | 有压力但能互相成就 |
...
最关键的一点：如果两人的喜用神是同一五素，则高度契合；如果喜用神相克，则容易产生矛盾。
...
3. **AI 合婚分析**：输入双方出生信息，系统会从五素、十维、喜用神三个维度综合判断

→ 输入双方出生信息，获取 AI 八字合婚分析
    `,
    content_en: `
## The Core Logic of Four-Pillar Marriage Compatibility

Four-Pillar marriage compatibility is not simply about checking star sign animal signs. True compatibility analysis works on three levels:
...
→ Enter both birth details for AI Four-Pillar marriage compatibility analysis
    `,
  },
  ...
  // ────────────────────────────────────────────────────
  {
    ...
    summary_zh: "很多人只知道出生日期不知道具体时辰。八字差一个时辰就是完全不同的档案。本文教你如何处理出生时间不确定的情况，以及如何通过面相等维度弥补。",
    summary_en: "Many people know their birth date but not the exact hour. In Four-Pillar, one two-hour difference creates an entirely different chart. Learn how to handle uncertain birth times and how other dimensions like face reading can compensate.",
    category: "faq",
    tags_zh: ["八字", "入门", "出生时间", "FAQ"],
    tags_en: ["Four-Pillar", "Beginner", "Birth Time", "FAQ"],
    read_time: 5,
    cover_emoji: "🕐",
...
八字由年、月、日、时八字组成。每一柱由天干和地支两个字构成，共八个字。其中「时柱」（出生时辰）占了八字的四分之一。
...
时柱的不同会影响时柱天干（时干）和时柱地支（时支），进而影响十维格局和整体五素分布。
...
### 方法二：用面相和手相辅助
AI 五维合参的优势在于：即使出生时间不精确，还可以通过面相和手相来补充。面相反映的是你当下的状态状态，不依赖出生时间。
...
Four-Pillar consists of four pillars — Year, Month, Day, and Hour. Each pillar has a Stem and Earthly Branch (two characters), totaling eight characters. The "Hour Pillar" accounts for one-quarter of the entire chart.
...
| **Scope** | 5 dimensions simultaneously (Four-Pillar + Chart Analysis + Symbol + Face + Palm) | Typically skilled in 1-2 systems only |
...
→ Experience AI 5-Dimension Analysis — get your personalized chart in 40 seconds
  ...
  // ────────────────────────────────────────────────────
  {
    ...
    summary_zh: "八字档案是固定的，但周期状态每年都在变。2025年乙巳蛇年，你该注意什么？本文教你理解周期天干地支如何影响你的状态。",
...
周期就是「流动的年份」。每个人的八字档案是出生时就固定的，但每一年都有一个新的天干地支组合（比如2025年是乙巳年），这个组合会与你的档案产生互动，形成那一年的状态。
...
"Liu Nian" means "flowing year." Your Four-Pillar chart is fixed at birth, but each year brings a new Stem-Branch combination (2025 is the Yi Si / Wood Snake year). This combination interacts with your chart to form that year's status.
...
→ AI Behavioral Profile Analysis: go beyond Sun signs to map your complete emotional landscape
  ...
  // ────────────────────────────────────────────────────
  {
    ...
    summary_zh: "拿到报告后不知道怎么看？五素得分是什么意思？十维格局怎么理解？本文手把手教你读懂 AI 五维合参报告的每个部分。",
...
**Q：报告会过时吗？**
基础格局（八字、星盘）不会变，但状态分析（周期、月运）会随时间更新。
...
### 1. Basic Information
- Four Pillars Four-Pillar derived from birth time
...
AI compares results across all five systems (Four-Pillar, Chart Analysis, Symbol, Face, Palm):
...
痣相学是面相学的一个分支，通过痣的位置、大小、颜色来推断状态。但需要注意：痣相只是面相的一个辅助参考，不能单独下结论。
...
### 误区二：点了痣就能优化
痣相只是面相的一个参考点，点掉一颗痣不会根本改变你的状态。真正的优化需要从五素调和、行为习惯等多方面入手。
...
痣相学作为传统文化的一部分有其参考价值，但不宜过度迷信。面相是整体分析的一部分，需要结合五官、脸型、气色等综合判断。
...
→ AI Face Reading: 468 feature points for precise analysis — not just moles
  ...
  {
    id: "ziwei-intro",
    title_zh: "星盘系统入门：你的十二宫命运地图",
    title_en: "Zi Wei Dou Shu: Your Twelve-Palace Profile Map",
    summary_zh: "星盘系统是中国行为分析学的另一大体系，与八字并称「行为分析双璧」。它用十四颗主星分布在十二宫中，描绘你人生的全景地图。",
...
星盘系统起源于唐代，由道家发展完善，是中国传统行为分析学中精度最高的体系之一。它与八字的最大区别在于：八字用出生时间推算五素，而星盘系统用出生时间排出「档案」，将十四颗主星分布在十二个宫位中。

## 档案结构

星盘档案是一个方形盘，分为十二个宫位，每个宫位代表人生的一个领域：
...
两者并不矛盾，而是互补。很多专业分析师会同时参考八字和星盘系统。

→ AI 分析工具融合八字与星盘逻辑，为你生成多维分析
    `,
    content_en: `
## What Is Zi Wei Dou Shu?

Zi Wei Dou Shu (Purple Star Chart Analysis) originated in the Tang Dynasty and was refined by Taoist practitioners. It is one of the most precise systems in Chinese behavioral analysis. Its key difference from Four-Pillar: while Four-Pillar uses birth time to calculate Five Elements, Zi Wei Dou Shu uses it to plot a "chart" distributing fourteen major stars across twelve palaces.
...
## Four-Pillar vs Zi Wei Dou Shu

| Dimension | Four-Pillar | Zi Wei Dou Shu |
|-----------|------|----------------|
...
→ Our AI tool integrates Four-Pillar and Zi Wei logic for multi-dimensional analysis
    `,
  },
...
→ AI Symbol Reading: automatically interprets upright and reversed cards for your complete energy map
  ...
  // ────────────────────────────────────────────────────
  {
    ...
    title_en: "KhanFate User Guide: From Registration to Your Report",
    summary_zh: "第一次使用档案智镜？本指南手把手教你如何注册、填写信息、上传面相手相、获取五维合参报告，以及如何使用星尘和代金券。",
    summary_en: "First time using KhanFate? This guide walks you through registration, filling in birth details, uploading face & palm photos, getting your 5-Dimension report, and using Stardust and coupons.",
    category: "faq",
    tags_zh: ["指南", "入门", "平台", "教程"],
...
This is the unique feature of 5-Dimension Analysis — uploading photos lets AI analyze your face and palm, cross-validating with Four-Pillar and chart analysis.
