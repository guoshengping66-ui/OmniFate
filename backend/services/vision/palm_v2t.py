...
class PalmV2T:
    ...
    def _describe_hand_shape(shape_ratio: float, finger_ratio: float,
                              color_desc: str) -> str:
        """
        Classify into Five Element hand shapes via Gaussian scoring.
    ...
    def _describe_fate_line(line_count: int, shape_ratio: float) -> str:
        ...
        if line_count > 10:
            return "命运线若隐若现或分段呈现（事业靠自力更生为主，转折点在35-40岁前后，中年后状态逐渐明朗。分段式命运线代表人生有多次自主选择的转折点）"
        if line_count > 6:
    ...
    def _estimate_wrist_lines(bgr_img: np.ndarray, lm, w: int, h: int,
        ...
        try:
            ...
            if band_count >= 3:
                return "手腕线清晰2-3条（福泽深厚，身体健康基础好，中年后状态有根基支撑）"
            elif band_count == 2:
    ...
    def _describe_special_marks(line_count: int, shape_ratio: float,
        ...
        if shape_ratio < 0.60:
            marks.append("掌心修长（水性能量主导，情感与直觉的接收器灵敏。对能量场敏感，适合冥想修行与艺术创作）")
        if line_count > 20:
    ...
    def _generate_summary(hand_shape: str, line_count: int,
        ...
        # Build report
        if score >= 5:
            base = "手相格局优良。主线清晰有力，掌丘饱满，掌色健康，人生各领域能量均衡流通。事业、感情、健康均有良好的先天基础，中年后状态呈上升趋势。"
        elif score >= 3:
            base = "手相格局中上。有强项亦有可提升之处，人生机遇与挑战并存。关键在35-45岁之间的转折期——主动调整则可顺势而上。"
        elif score >= 1:
            base = "手相格局中等。先天禀赋有强有弱，需通过后天努力补足短板——这正是此命给予你的成长空间。掌色与掌丘偏弱者建议配合后天调整。"
        else:
            base = "手相偏复杂，先天能量分布不均衡。人生历练丰富，需通过后天修为和调整扬长避短。后天补益对此命而言尤为重要。"
