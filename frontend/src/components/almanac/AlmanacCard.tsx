          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="text-xs text-white/45">{isZh ? "今日主线" : "Throughline"}: {trend.focusArea}</span>
              </div>
              <h2 className="mt-4 max-w-2xl font-serif text-2xl font-bold leading-tight text-white md:text-3xl">
                {trend.headline}
              </h2>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-white/62">{trend.summary}</p>
            </div>

            <div className="w-full rounded-xl border border-white/[0.08] bg-black/20 p-4 lg:w-40">
              <div className="flex items-end justify-between lg:block">
                <div>
                  <p className="text-xs text-white/42">{isZh ? "趋势分" : "Trend score"}</p>
                  <p className="mt-1 font-serif text-4xl font-bold text-gold">{trend.score}</p>
                </div>
                <p className="text-xs text-white/42 lg:mt-1">/100</p>
              </div>
              <div className="mt-3">
          ...
          <main className="space-y-5 border-white/[0.08] p-5 md:p-7 lg:border-r">
            <div className="grid gap-3 md:grid-cols-2">
              <div className="rounded-xl border border-white/[0.08] bg-[#030918] p-4">
                ...
                <p className="text-sm leading-6 text-white/62">{trend.focusArea}</p>
              </div>
              ...
              <div className="rounded-xl border border-amber-300/20 bg-amber-300/[0.055] p-4">
                ...
                <p className="text-sm leading-6 text-white/62">{trend.riskSignal}</p>
              </div>
            </div>
            ...
            <div>
              <div className="mb-3 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-white/82">{isZh ? "今天就做这三件事" : "Three useful actions"}</h3>
              </div>
              <div className="space-y-2.5">
                {trend.actions.map((action, index) => (
                  <div key={action} className="flex gap-3 rounded-lg border border-white/[0.08] bg-[#030918] p-3">
                    ...
                    <p className="text-sm leading-5 text-white/72">{action}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/[0.045] p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  )) : <span className="text-xs text-white/42">{isZh ? "保持当前节奏" : "Keep the current rhythm"}</span>}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-rose-400/20 bg-rose-400/[0.04] p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  )) : <span className="text-xs text-white/42">{isZh ? "暂无明显禁忌" : "No strong avoid signal"}</span>}
                </div>
              </div>
            </div>
          </main>
          ...
          <aside className="space-y-5 p-5 md:p-7">
            <div>
              <div className="mb-3 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-white/82">{isZh ? "分时节奏" : "Daily rhythm"}</h3>
              </div>
            ...
            <div>
              <div className="mb-3 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-white/82">{isZh ? "四项状态" : "Signal map"}</h3>
              </div>
              <div className="space-y-3">
                {trend.dimensions.map((dimension) => (
                  <div key={dimension.label}>
                    <div className="mb-1.5 flex items-center justify-between gap-3 text-xs">
                      <span className="text-white/62">{dimension.label}</span>
                      <span className="text-white/42">{dimension.note} · {dimension.value}</span>
                    </div>
            ...
            {(trend.wuxingAnalysis || trend.dailyQuote || trend.hu.length > 0) && (
              <div className="rounded-xl border border-white/[0.08] bg-[#030918] p-4">
                ...
                {trend.wuxingAnalysis && <p className="text-xs leading-5 text-white/52">{trend.wuxingAnalysis}</p>}
                ...
                {trend.hu.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {trend.hu.map((item, index) => (
                      <p key={`${item.label}-${index}`} className="text-xs leading-5 text-white/42">
