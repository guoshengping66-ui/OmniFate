...
export class ErrorBoundary extends React.Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div className="card-solid p-6 my-4">
          <div className="flex items-center gap-3 mb-3">
            <AlertCircle className="text-red-400" size={20} />
            <h3 className="text-parchment-200 text-sm font-medium">
              {this.props.sectionName
                ? `${this.props.sectionName} failed to load`
                : "Something went wrong"}
            </h3>
          </div>
          <p className="text-parchment-400 text-xs mb-4">
            {this.state.error?.message || "An unexpected error occurred"}
          </p>
          <button
            onClick={() => this.setState({ hasError: false, error: null })}
            className="flex items-center gap-2 px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-parchment-300 text-xs transition-colors"
          >
            <RefreshCw size={14} />
            Try again
          </button>
