...
async function proxy(request: Request, params: Promise<{ path: string[] }>) {
  ...
  try {
    ...
    // Read body — use text() for correct UTF-8 handling of Chinese characters.
    // Client-side proxies (Clash/V2Ray) can mangle UTF-8 bytes in POST bodies;
    // text() handles encoding more robustly than arrayBuffer().
