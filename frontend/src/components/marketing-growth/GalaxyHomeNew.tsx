    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
      {Array.from({ length: 24 }, (_, i) => (
        <div key={i} className="absolute rounded-full" style={{
          ...
          top: `${(i * 53 + 7) % 85 + 5}%`,
          left: `${(i * 37 + 11) % 100}%`,
          background: i % 6 === 0 ? "rgba(201,168,76,0.5)" : "rgba(190,195,220,0.35)",
          boxShadow: i % 6 === 0 ? "0 0 6px rgba(201,168,76,0.4)" : "0 0 3px rgba(180,185,210,0.25)",
          opacity: 0.2,
  ...
  return (
    <main className="relative min-h-screen overflow-hidden text-white" style={{ background: "#020617" }}>
      ...
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true" style={{
        background:
          "radial-gradient(ellipse 80% 60% at 50% 30%, rgba(123,158,199,0.15) 0%, transparent 60%)," +
      ...
      {/* Layer 4: Bagua */}
      <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none" aria-hidden="true" style={{ opacity: 0.28 }}>
        <div className="relative" style={{ width: "min(82vw, 520px)", height: "min(82vw, 520px)" }}>
          <div className="absolute inset-0 rounded-full border border-white/[0.03]" style={ringSlow} />
          <div className="absolute inset-[5%]" style={ringSlow}>
            {TRIGRAMS.map((t, i) => {
              const a = (i / 8) * 360
              return <div key={i} className="absolute text-[10px] text-white/25 font-serif" style={{ left:"50%", top:"50%", transform:`rotate(${a}deg) translateY(-47%) rotate(-${a}deg) translateX(-50%)` }}>{t}</div>
            })}
          </div>
          <div className="absolute inset-[12%] rounded-full border border-white/[0.04]" style={ringRev} />
          <div className="absolute inset-[18%]" style={ringMed}>
            <div className="absolute inset-0 rounded-full border border-white/[0.05]" />
            {TRIGRAMS.map((t, i) => {
              const a = (i / 8) * 360 + 22.5
              return <div key={i} className="absolute text-xs text-white/18 font-serif" style={{ left:"50%", top:"50%", transform:`rotate(${a}deg) translateY(-${i%2===0?"37%":"34%"}) rotate(-${a}deg) translateX(-50%)` }}>{t.slice(1)}</div>
            })}
          </div>
          <div className="absolute inset-[26%] rounded-full border border-white/[0.04]" style={ringFast} />
          ...
          <div className="absolute inset-[34%] rounded-full" style={{ background:"radial-gradient(circle, rgba(201,168,76,0.05) 0%, transparent 70%)" }} />
          <div className="absolute inset-[36%] rounded-full flex items-center justify-center">
            <div className="absolute inset-[-10%] rounded-full border border-white/[0.05]" style={coreStyle} />
            <span className="text-3xl text-white/25 select-none" style={coreStyle}>☯</span>
          </div>
        </div>
      </div>
