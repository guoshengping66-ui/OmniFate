...
function mkS() {
  const r1 = srng(191), r2 = srng(377), r3 = srng(523); const s: Array<{ id: string; x: number; y: number; sz: number; o: number; tw: boolean; sp: number; dl: number; clr: "warm" | "cool" | "blue" }> = []
...
function mkQ() { return Array.from({ length: 40 }, (_, i) => { const a = (i / 40) * 360, d = 15 + (i % 6) * 6; return { id: i, ang: a, dist: d, sp: 2 + (i % 6) * 1.2, dl: i * .5, sz: 1.2 + (i % 6) * .6 } }) }
const SYS = [{ n: "八字", nE: "Bazi", c: "#5A9E8E", f: !0 }, { n: "紫微", nE: "Ziwei", c: "#8B7EC7", f: !0 }, { n: "星盘", nE: "Astrology", c: "#7B9EC7", f: !0 }, { n: "塔罗", nE: "Tarot", c: "#C77B8B", f: !1 }, { n: "面相", nE: "Face", c: "#C4BFB0", f: !1 }, { n: "手相", nE: "Palm", c: "#C9A84C", f: !1 }]
const INP = { zh: ["生辰八字", "出生地点", "面相照片", "手相照片", "当前问题", "生活照片"], en: ["Birth date & time", "Birth location", "Face photo", "Palm photo", "Your question", "Lifestyle photo"] }
const OUT = { zh: ["性格结构", "事业方向", "关系模式", "财富窗口", "今日行动", "生活方式"], en: ["Personality", "Career", "Relationships", "Wealth window", "Daily action", "Lifestyle"] }
...
@media(prefers-reduced-motion:reduce){*{animation:none!important}}
    ...
    <section className="relative flex min-h-[90vh] w-full flex-col items-center justify-center px-6 text-center" style={{ zIndex: 10 }}>
      <div style={{ animation: "fadeUp 0.8s ease-out forwards" }}>
        <h1 className="font-serif text-9xl md:text-11xl font-bold tracking-wide" style={{ background: "linear-gradient(180deg, #f5e0a0 0%, #c49a35 48%, #7d5a10 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", filter: "drop-shadow(0 0 18px rgba(201,168,76,0.15))" }}>{isZh ? "KhanFate" : "KhanFate"}</h1>
