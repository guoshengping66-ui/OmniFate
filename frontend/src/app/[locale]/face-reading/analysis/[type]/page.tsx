...
import { FaceAnalyses } from "@/data/programmatic/face/analysis"
...
export async function generateStaticParams() {
  return FaceAnalyses.map(a => ({ type: a.id }))
}

export async function generateMetadata({ params }: PageProps) {
  const { locale, type } = await params
  const data = FaceAnalyses.find(a => a.id === type)
  if (!data) return {}
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.khanfate.com/${locale}/face-reading/analysis/${type}`,
      languages: {
        en: `https://www.khanfate.com/en/face-reading/analysis/${type}`,
        zh: `https://www.khanfate.com/zh/face-reading/analysis/${type}`,
        "x-default": `https://www.khanfate.com/en/face-reading/analysis/${type}`,
      },
    },
  }
}

export default async function FaceAnalysisPage({ params }: PageProps) {
  const { locale, type } = await params
  const data = FaceAnalyses.find(a => a.id === type)
  if (!data) notFound()
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 px-4">
      <div className="max-w-4xl mx-auto">
        <Breadcrumbs
          items={[
            { label: isZh ? "面相" : "Face Reading", href: "/face-reading" },
            { label: isZh ? data.name_zh : data.name_en },
          ]}
          currentPath={`/${locale}/face-reading/analysis/${type}`}
        />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{ __html: safeJsonLd({
            ...
            "description": isZh ? data.meta_description_zh : data.meta_description_en,
            "url": `https://www.khanfate.com/${locale}/face-reading/analysis/${type}`,
          })}}
        />
        ...
        <ScrollReveal delay={0.2}>
          <div className="card-solid p-8 md:p-10 mb-8">
            ...
            <p className="text-parchment-400 text-sm leading-relaxed">{isZh ? data.indicators_zh : data.indicators_en}</p>
          </div>
        </ScrollReveal>
