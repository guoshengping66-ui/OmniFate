...
async def unlock_report(
    ...
    logger.info(f"[UNLOCK] reading_id={reading_id}, source_param={source!r}, user={current_user.id}")
    logger.info(f"[UNLOCK] raw_url={raw_url}, query_params={query_params}")
    ...
    if source == "stardust":
        logger.info(f"[UNLOCK] ★ ENTERING STARDUST BRANCH ★ reading={reading_id}")
    ...
    logger.info(f"[UNLOCK] ★ PAYMENT BRANCH ★ source={source!r} (stardust branch was skipped)")
    import sys; sys.stdout.flush()
