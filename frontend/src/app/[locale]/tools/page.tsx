        ...
        <ScrollReveal delay={0.05}>
          <div className="mb-6 border border-gold/20 bg-gold/[0.04] p-5">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-gold/60">
                  Recommended path
                </p>
              ...
              <Link href={localeHref("/reading/new?intent=full")} className="btn-primary inline-flex items-center justify-center gap-2 px-5 py-3 text-sm">
          ...
          {tools.map((tool) => (
            <ScrollReveal key={tool.href} delay={0.1}>
              <Link
                ...
                className="card-interactive group block p-6 transition-all duration-300 hover:border-gold/30"
              >
                ...
                <div className="relative flex items-start gap-4">
                  <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center border border-white/[0.06] bg-white/[0.04] text-gold transition-colors group-hover:border-gold/30">
                    <tool.icon size={20} />
                  </div>
                  <div className="flex-1">
                    <div className="mb-2 flex flex-wrap items-center gap-2">
                      <h3 className="font-serif font-bold text-gold transition-colors group-hover:text-gold-light">{tool.title}</h3>
                      {tool.tag && <span className="border border-white/[0.06] px-2 py-0.5 text-xs uppercase tracking-[0.16em] text-parchment-400">{tool.tag}</span>}
                    </div>
                    <p className="text-sm leading-relaxed text-parchment-400">{tool.desc}</p>
