...
class FaceV2T:
    ...
    def _describe_three_zones(f: float, n: float, c: float) -> str:
        ...
        if abs(f - n) < 0.05 and abs(n - c) < 0.05:
            balance = "三庭均等（格局平稳，早中晚年状态均衡）"
        elif f > n and f > c:
            balance = "上停偏长（早年运好，贵人助力强，智慧出众）"
        elif c > f and c > n:
            balance = "下停偏长（晚年福泽深，积累能力强）"
        elif n > f and n > c:
            balance = "中停偏长（中年状态最旺，事业财富巅峰在中年）"
        else:
    ...
    def _describe_forehead(f_ratio: float, rel_h: float) -> str:
        ...
        if f_ratio > 0.30:
            return "额头适中端正（早年状态稳健，依靠自身努力取得成就）"
    ...
    def _generate_summary(f: float, n: float, c: float,
        ...
        if pct >= 0.85:
            return "面相格局上上，五官配合得天独厚，一生状态顺畅，贵气盈门，宜善加珍惜。"
        if pct >= 0.70:
            return "面相格局上等，五官协调有力，主一生状态通达，中晚年积累丰厚，贵气可期。"
        if pct >= 0.50:
            return "面相格局中等，五官强弱互现，需针对性强化薄弱宫位对应的能力，方能趋利避害。"
        if pct >= 0.35:
            return "面相格局中下，五官偏弱处较多，建议通过后天调整与习惯养成补强先天不足，后天修为尤为关键。"
        return "面相格局偏弱，五官配合度不足，需全面调理，建议结合数据综合分析，以定化解之策。"
