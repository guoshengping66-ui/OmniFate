...
export function ZiweiPalaceTemplate({ data, locale }: ZiweiPalaceTemplateProps) {
    ...
    <div className="min-h-screen pt-24 pb-20 px-4">
      <div className="max-w-4xl mx-auto">
        <Breadcrumbs
          ...
          currentPath={`/${locale}/ziwei/palaces/${data.id}`}
        />
        ...
        <ScrollReveal delay={0.25}>
          <div className="card-glass p-8 md:p-10 mb-8">
            ...
            <p className="text-white/60 text-sm leading-relaxed">{content.fortune}</p>
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal delay={0.4}>
          <div className="card-glass p-8 md:p-10 mb-8">
            <h2 className="font-serif text-2xl text-gold mb-6">
              {isZh ? "探索其他宫位" : "Explore Other Palaces"}
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {data.related_palaces.map(palace => {
                const relatedPalace = ZiweiPalaces.find(p => p.id === palace)
                return (
                  <Link
                    key={palace}
                    href={localeHref(`/ziwei/palaces/${palace}`)}
                    className="card-glow p-3 text-center hover:border-gold/30 transition-all group"
                  >
                    <div className="text-2xl mb-1">{relatedPalace?.emoji}</div>
                    <div className="text-xs text-white/60 group-hover:text-gold transition-colors">
                      {isZh ? relatedPalace?.name_zh : relatedPalace?.name_en}
                    </div>
                  </Link>
                )
              })}
