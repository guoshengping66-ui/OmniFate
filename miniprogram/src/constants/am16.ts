...
export const DIMENSIONS: Dimension[] = [
  { code: "FD", letterA: "F", letterB: "D", nameA: "顺天躺平", nameB: "物理逆天" },
  { code: "XS", letterA: "X", letterB: "S", nameA: "心觉玄测", nameB: "硬核格物" },
  { code: "GI", letterA: "G", letterB: "I", nameA: "红尘渡人", nameB: "闭门修仙" },
  { code: "PE", letterA: "P", letterB: "E", nameA: "稳如老狗", nameB: "知行合一" },
]
...
export const PERSONALITIES: Record<string, Personality> = {
  FXGP: {
    code: "FXGP", title: "心学派顶级躺平咸鱼", emoji: "🐟",
    quote: "此心光明，亦复何言",
    ...
    compatible: ["DXIE", "FSGP"], clash: ["DXGE", "FXGE"],
  },
  FXGE: {
    code: "FXGE", title: "玄学界佛系社交蝴蝶", emoji: "🦋",
    quote: "知行合一，方为真知",
    ...
    compatible: ["DXIE", "FSGP"], clash: ["DXGE", "FSIE"],
  },
  FXIP: {
    code: "FXIP", title: "断网修仙派高维咸鱼", emoji: "🧘",
    quote: "无善无恶心之体",
    ...
    compatible: ["DXGP", "FSGP"], clash: ["FXGE", "DXGE"],
  },
  FXIE: {
    code: "FXIE", title: "靠直觉蹦迪的玄学交际花", emoji: "💫",
    quote: "良知即是独知时",
    ...
    compatible: ["DXGE", "FSGP"], clash: ["FSIE", "DXIP"],
  },
  FSGP: {
    code: "FSGP", title: "看盘等死的数字居士", emoji: "📈",
    quote: "去人欲，存天理",
    ...
    compatible: ["DXIE", "DXGE"], clash: ["DXGP", "FXIE"],
  },
  FSGE: {
    code: "FSGE", title: "赛博功德箱固定传声筒", emoji: "📱",
    quote: "致良知于事事物物",
    ...
    compatible: ["DXIE", "FXGP"], clash: ["DXGE", "FXGE"],
  },
  FSIP: {
    code: "FSIP", title: "Excel 表格算命隐士", emoji: "📊",
    quote: "人心之得其正者即道",
    ...
    compatible: ["DXGE", "FXGP"], clash: ["FXGE", "DXGE"],
  },
  FSIE: {
    code: "FSIE", title: "孤独的量子纠缠改命极客", emoji: "🧑‍🔬",
    quote: "知是行之始，行是知之成",
    ...
    compatible: ["FXGP", "DXIE"], clash: ["FXGE", "DXGE"],
  },
  DXGP: {
    code: "DXGP", title: "网络逆天口嗨/肉身躺平侠", emoji: "🎭",
    quote: "知是行之始，行是知之成",
    ...
    compatible: ["FXGP", "FSIE"], clash: ["DXIE", "FXIE"],
  },
  DXGE: {
    code: "DXGE", title: "红尘蹦迪的改运队长", emoji: "🎉",
    quote: "人须在事上磨",
    ...
    compatible: ["FXGP", "FSIP"], clash: ["FXGE", "FSIE"],
  },
  DXIP: {
    code: "DXIP", title: "反骨入命的直觉独狼", emoji: "⚔️",
    ...
    compatible: ["FXIP", "FSIE"], clash: ["FXGE", "FSIP"],
  },
  DXIE: {
    code: "DXIE", title: "凌晨三点与天对线狂人", emoji: "🔥",
    quote: "破山中贼易，破心中贼难",
  ...
  DSIE: {
    code: "DSIE", title: "逆天改命硬核特种兵", emoji: "⚡",
    quote: "知行并进，不可分作两事",
    ...
    compatible: ["FXGP", "FXIP"], clash: ["FXGE", "FSIE"],
  },
  DSGP: {
    code: "DSGP", title: "满脑子反骨的数据拖延症", emoji: "📋",
    quote: "静处体悟，事上磨练",
    ...
    compatible: ["DXIE", "FXGP"], clash: ["FXGE", "DXGE"],
  },
  DSGE: {
    code: "DSGE", title: "全栈型改命运维工程师", emoji: "🛠️",
    quote: "知行并进，功夫不离本体",
  ...
  DSIP: {
    code: "DSIP", title: "莫得感情的格物修仙独狼", emoji: "🐺",
    quote: "此心不动，随机而动",
...
type Polarity = "A" | "B"
type DimKey = "FD" | "XS" | "GI" | "PE"

interface DimScore { A: number; B: number }

const Q_DIM_MAP: Record<number, DimKey> = {
  1: "FD", 2: "FD", 3: "FD",
  4: "XS", 5: "XS", 6: "XS",
  7: "GI", 8: "GI", 9: "GI",
  10: "PE", 11: "PE", 12: "PE",
...
const LETTER_TO_POLARITY: Record<string, Polarity> = {
  F: "A", D: "B", X: "A", S: "B",
  G: "A", I: "B", P: "A", E: "B",
}
...
/**
 * 核心计分函数
 * @param answers { 题号: 'A' | 'B' | 'C' }
 * @returns 4 位字母编码（如 "DXIE"）
 *
 * 平局容灾：绝对平局时默认返回 A 极（F/X/G/P），
 * 确保 100% 拼接出 4 位大写字母编码，绝不崩溃。
 */
export function calculateArchetype(answers: Record<number, "A" | "B" | "C">): string {
  const scores: Record<DimKey, DimScore> = {
    FD: { A: 0, B: 0 },
    XS: { A: 0, B: 0 },
    GI: { A: 0, B: 0 },
    PE: { A: 0, B: 0 },
  ...
  for (const q of QUESTIONS) {
    ...
    const dimKey = Q_DIM_MAP[q.id]
    if (!dimKey) continue

    const idx = answer === "A" ? 0 : answer === "B" ? 1 : 2
    const opt = q.options[idx]

    const pol = LETTER_TO_POLARITY[opt.dimension]
    if (pol) scores[dimKey][pol] += opt.points

    if (opt.altDimension) {
      const altPol = LETTER_TO_POLARITY[opt.altDimension]
      if (altPol) scores[dimKey][altPol] += opt.points
    }
  }

  const dims: { code: DimKey; a: string; b: string }[] = [
    { code: "FD", a: "F", b: "D" },
    { code: "XS", a: "X", b: "S" },
    { code: "GI", a: "G", b: "I" },
    { code: "PE", a: "P", b: "E" },
  ...
  return dims.map(d => {
    ...
    // 平局容灾：绝对平局 → 默认 A 极（F/X/G/P）
    if (s.A === s.B) return d.a
...
export function calculateRadarScores(answers: Record<number, "A" | "B" | "C">): Record<string, number> {
  const scores: Record<DimKey, DimScore> = {
    FD: { A: 0, B: 0 }, XS: { A: 0, B: 0 },
    GI: { A: 0, B: 0 }, PE: { A: 0, B: 0 },
  }
  ...
  const dimCodes: DimKey[] = ["FD", "XS", "GI", "PE"]
  for (const dc of dimCodes) {
    const total = scores[dc].A + scores[dc].B
    radar[dc] = total > 0 ? Math.round((scores[dc].B / total) * 100) : 50
  }
  return radar
}
