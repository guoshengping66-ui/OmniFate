  ...
  return (
    <div className="card-interactive p-6 relative overflow-hidden">
      ...
      <div className="relative flex items-center gap-4">
        ...
        <div>
          <p className="text-parchment-400 text-xs tracking-wider uppercase mb-1">{t("billing.balance")}</p>
          <p className="text-3xl font-bold text-gold font-serif">{balance.toLocaleString()}</p>
          <p className="text-parchment-400 text-xs mt-1">{t("billing.balanceDesc")}</p>
        </div>
      </div>
    </div>
  )
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="card-solid p-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-parchment-300 text-sm font-medium">Stripe payments only</span>
        </div>
