    ...
    <div className={`space-y-4 ${className}`}>
      ...
      >
        <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${
          ...
          {isSelected && <Check size={10} className="text-cosmos-950 anim-scale-in" strokeWidth={3} />}
        </div>
        <CreditCard size={18} className={isSelected ? "text-gold" : "text-parchment-400"} />
        <span className={`text-sm font-medium ${isSelected ? "text-gold" : "text-parchment-300"}`}>
          {displayName}
        </span>
