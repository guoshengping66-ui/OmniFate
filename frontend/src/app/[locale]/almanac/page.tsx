...
export default function AlmanacPage() {
    ...
    return (
      <div className="min-h-screen pt-24 pb-16 px-4 flex items-center justify-center">
        <div className="card-solid p-10 text-center max-w-md">
          ...
          <button onClick={() => router.push(localeHref("/pricing"))} className="btn-primary">
            {t("almanac.viewPlans")}
          </button>
        </div>
      </div>
  ...
  if (authLoading || loading) return (
    <div className="min-h-screen pt-24 pb-16 px-4 flex items-center justify-center">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-parchment-400 text-sm">{t("almanac.loading")}</p>
      </div>
    </div>
  )
  ...
  if (noSession) return (
    <div className="min-h-screen pt-24 pb-16 px-4 flex items-center justify-center">
      <div className="card-solid p-10 text-center max-w-md">
        <p className="text-parchment-400 text-sm mb-4">{t("almanac.needReading")}</p>
        <button onClick={() => router.push(localeHref("/reading/new"))} className="btn-primary">
  ...
  if (error) return (
    <div className="min-h-screen pt-24 pb-16 px-4 flex items-center justify-center">
      <div className="card-solid p-10 text-center max-w-md">
        ...
        <h3 className="font-serif text-lg text-parchment-200 mb-2">{t("almanac.loadError")}</h3>
        <p className="text-parchment-400 text-xs mb-1 break-all">{error}</p>
        <button
  ...
  if (!data) return (
    <div className="min-h-screen pt-24 pb-16 px-4 flex items-center justify-center">
      <p className="text-parchment-400">{t("almanac.noData")}</p>
    </div>
  )
