...
export function SampleGrowthReport() {
  ...
  return (
    <section id="sample-growth-report" className="px-4 py-20 sm:py-28">
      <div className="mx-auto max-w-6xl border border-white/[0.06] bg-[#0A1017] p-4 sm:p-8">
        <div className="mb-8 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-gold/60">Sample Report</p>
          ...
          <div className="flex border border-white/[0.06] bg-black/20 p-1">
            {copy.reportTabs.map((tab) => (
              <button
                ...
                className={`px-4 py-2 text-sm transition-colors ${
                  active.key === tab.key ? "bg-gold text-black" : "text-parchment-300 hover:text-white"
                }`}
              >
                {tab.label}
              </button>
        ...
        <div className="grid gap-6 lg:grid-cols-[0.92fr_1.08fr]">
          <div className="border border-gold/15 bg-gold/[0.045] p-6">
            <p className="text-xs uppercase tracking-[0.18em] text-gold/60">Growth Archetype</p>
            <h3 className="mt-4 text-2xl font-semibold text-white">{active.title}</h3>
            <p className="mt-4 text-base leading-8 text-parchment-300">{active.body}</p>
          </div>
          <div className="grid gap-3">
            {active.bullets.map((bullet) => (
              <div key={bullet} className="flex gap-3 border border-white/[0.06] bg-white/[0.025] p-4">
                <Check className="mt-0.5 shrink-0 text-gold" size={18} />
                <p className="text-sm leading-6 text-parchment-300">{bullet}</p>
              </div>
            ))}
          </div>
        </div>
