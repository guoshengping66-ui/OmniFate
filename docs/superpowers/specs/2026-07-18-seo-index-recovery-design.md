...
2. Make the article layout call `notFound()` for an unknown article and serve `noindex` plus the canonical English URL when an article is unavailable in a requested locale.
