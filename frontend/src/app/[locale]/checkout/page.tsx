  ...
  if (items.length === 0 && !done) {
    return (
      <div className="min-h-screen pt-24 pb-16 px-4 text-center">
        ...
        <p className="text-parchment-400">{t("checkout.empty")}</p>
  ...
  if (done) {
    return (
      <div className="min-h-screen pt-24 pb-16 px-4 flex items-center justify-center">
        <div className="card-solid p-10 text-center max-w-md">
          <div className="w-16 h-16 rounded-full bg-green-500/20 border border-green-500/40 flex items-center justify-center mx-auto mb-4">
            <CheckCircle size={32} className="text-green-400" />
          </div>
          <h2 className="font-serif text-xl text-gold mb-2">{t("checkout.orderSuccessTitle")}</h2>
          <p className="text-parchment-400 text-sm mb-4">{t("checkout.thankYou")}</p>
          <div className="bg-white/[0.04] border border-white/[0.06] rounded-lg p-4 mb-6 text-left">
            <p className="text-parchment-400 text-xs mb-2">{t("checkout.nextSteps")}:</p>
            <ul className="text-parchment-400 text-xs space-y-1.5">
          ...
          <div className="flex gap-3">
            <button onClick={() => router.push(localeHref("/account") + "?tab=orders")} className="btn-primary flex-1 text-sm">
              {t("checkout.viewOrders")}
            </button>
            <button onClick={() => router.push(localeHref("/shop"))} className="btn-primary flex-1 text-sm">
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Order items */}
        <div className="card-solid p-6 mb-4 space-y-3">
          {items.map(item => (
            <div key={item.product.id} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-white/[0.04] flex items-center justify-center text-lg">
                  🎁
                </div>
                <div>
                  <p className="text-parchment-200 text-sm">{locale === "en" ? (item.product.name_en || item.product.name) : item.product.name}</p>
                  <p className="text-parchment-400 text-xs">x{item.quantity}</p>
                </div>
              </div>
              <span className="text-parchment-300 text-sm">{symbol}{(getItemPrice(item.product) * item.quantity).toFixed(2)}</span>
            </div>
          ))}
        </div>
          ...
          <div className="card-solid p-4 mb-4">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <div className="flex-1">
                <span className="text-parchment-200 text-sm">{t("checkout.coupon")}</span>
                <span className="text-parchment-400 text-xs ml-2">
        ...
        {/* Totals */}
        <div className="card-solid p-6 mb-6 space-y-3">
          ...
          {isOverseas && shippingFee > 0 && (
            <p className="text-parchment-400 text-xs text-right -mt-1">
              {t("checkout.freeShippingHint")}
            </p>
          )}
          <div className="border-t border-white/[0.06] pt-3 flex justify-between">
            <span className="text-parchment-200 font-medium">{t("checkout.total")}</span>
        ...
        <div className="card-solid p-6 mb-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-parchment-300 text-sm font-medium">{t("checkout.shippingAddress")}</span>
          </div>
          <AddressForm
        ...
        <div className="card-solid p-6 mb-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-parchment-300 text-sm font-medium">Stripe</span>
          </div>
        ...
        <label className="flex items-start gap-3 cursor-pointer mb-4">
          <input
            ...
            className="mt-0.5 h-4 w-4 rounded border-white/20 bg-white/[0.04] text-gold focus:ring-gold/40 shrink-0"
          />
          <div className="text-parchment-400 text-xs leading-relaxed">
        ...
        <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-3 mb-4">
          <p className="text-parchment-400 text-xs leading-relaxed text-center">
